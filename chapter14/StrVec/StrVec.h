...
class StrVec
{
    friend bool operator==(const StrVec &,const StrVec &);  //practice14.16
    friend bool operator!=(const StrVec &,const StrVec &);

    friend bool operator<(const StrVec &,const StrVec &);  //practice14.18
    friend bool operator>(const StrVec &,const StrVec &);
    friend bool operator<=(const StrVec &,const StrVec &);
    friend bool operator>=(const StrVec &,const StrVec &);
...
public:
    ...
    StrVec& operator=(std::initializer_list<std::string>);  //14.4赋值运算符同时也是practice14.23

    //14.5下标运算符同时也是practice14.26的作业
    //非常量版本
    ...
    //常量版本
    const std::string &operator[](std::size_t n) const  //后面这个const说明当前对象是const
    {
        return elements[n];
    }
