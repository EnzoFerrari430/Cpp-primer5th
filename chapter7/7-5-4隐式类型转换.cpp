...
public:
    ...
    Sales_data(string bookname):bookNo(bookname),units_sold(0),revenue(0){}
    bool isSameISBN(const Sales_data &r)
    {
...
int main()
{
    ...
    Sales_data itemb("bbb");


    itema.combine(string("ccc"));
    cout<<itema.isSameISBN(itemb)<<endl;  //正常比较
    cout<<itema.isSameISBN(string("bbb"))<<endl;  //隐式转换   string型转换成Sales_data型
    cout<<itema.isSameISBN(Sales_data("bbb"))<<endl;  //创建临时对象
    return 0;
}
